...
abstract class AbstractCLI(name: String, help: String) :
    ...
    companion object {
        ...
        fun mapperHelp(from: String, to: String) =
    ...
    open fun idToFileOption() =
        saveFileOption(
            LONGNAME_ID_TO_FILE,
            mapperHelp("id", "file"), File(resultDir, "idToFile.json")
        )

    open fun fileToIdOption() =
        saveFileOption(
            LONGNAME_FILE_TO_ID,
            mapperHelp("file", "id"), File(resultDir, "fileToId.json")
        )

    open fun idToUserOption() =
        saveFileOption(
            LONGNAME_ID_TO_USER,
            mapperHelp("id", "user"), File(resultDir, "idToUser.json")
        )

    open fun userToIdOption() =
        saveFileOption(
            LONGNAME_USER_TO_ID,
            mapperHelp("user", "id"), File(resultDir, "userToId.json")
        )

    open fun idToCommitOption() =
        saveFileOption(
            LONGNAME_ID_TO_COMMIT,
            mapperHelp("id", "commit"), File(resultDir, "idToCommit.json")
        )

    open fun commitToIdOption() =
        saveFileOption(
            LONGNAME_COMMIT_TO_ID,
            mapperHelp("commit", "id"), File(resultDir, "commitToId.json")
        )
